...
const _WRAPPER_PARAMETER_NAMES = keys(_WRAPPER_DEFAULTS)

function _without_wrapper_parameters(parameters::NamedTuple)
    names = Tuple(name for name in keys(parameters) if !(name in _WRAPPER_PARAMETER_NAMES))
...
) = _without_wrapper_parameters(overrides)

function _wrapper_settings(parameters::NamedTuple)
    merged = merge(_WRAPPER_DEFAULTS, parameters)
    settings = NamedTuple{_WRAPPER_PARAMETER_NAMES}(
        Tuple(getproperty(merged, name) for name in _WRAPPER_PARAMETER_NAMES)
    )
    all(value -> value isa Real && !(value isa Bool) && isfinite(value) && value >= 0,
        values(settings)) || throw(ArgumentError("FrankenLOBSTER wrapper parameters must be finite and nonnegative"))
    settings.carbon_ratio > 0 || throw(ArgumentError("carbon_ratio must be > 0"))
    settings.zooplankton_calcium_carbonate_dissolution <= 1 || throw(ArgumentError(
        "zooplankton_calcium_carbonate_dissolution must be <= 1"
    ))
    return settings
end
...
function _wrap_plankton(runtime, realization, parameters, ::Type{T}) where T
    settings = _wrapper_settings(parameters)
    ...
    return FrankenLOBSTERPlankton(
        ...
        chlorophyll_ratio=convert(T, settings.phytoplankton_chlorophyll_ratio),
        carbon_ratio=convert(T, settings.carbon_ratio),
        calcium_carbonate_rain_ratio=convert(T, settings.calcium_carbonate_rain_ratio),
        zooplankton_calcium_carbonate_dissolution=convert(
            T, settings.zooplankton_calcium_carbonate_dissolution
        ),
    )
end

function _construct_plankton(
    realization, parameters; grid=nothing, sinking_tracers=nothing, open_bottom=true,
)
    ...
    runtime = Construction.construct(
        ...
        grid,
        parameter_overrides=_without_wrapper_parameters(parameters),
        sinking_tracers,
        open_bottom,
        diagnostic_processes=_CALCITE_DIAGNOSTIC_PROCESSES,
    )
    T = isnothing(grid) ? Float64 : eltype(grid)
    return _wrap_plankton(runtime, realization, parameters, T)
end
...
function construct(;
    ...
    return _construct_plankton(realization, parameters; grid, sinking_tracers, open_bottom)
end
...
function construct(recipe::Construction.ModelRecipe; grid=nothing)
    ...
    T = isnothing(grid) ? Float64 : eltype(grid)
    return _wrap_plankton(runtime, recipe.plankton_pfts, recipe.parameter_overrides, T)
end
