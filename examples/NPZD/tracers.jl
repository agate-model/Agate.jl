...
tracers = Dict(
    ...
    "P" => :(
        idealized_photosynthetic_growth(N, P, PAR, μ₀, kₙ, α) - idealized_predation_loss(P, Z, gₘₐₓ, kₚ) -
        linear_loss(P, lᵖⁿ) - linear_loss(P, lᵖᵈ)
    ),
    "Z" =>
        :(idealized_predation_gain(P, Z, β, gₘₐₓ, kₚ) - linear_loss(Z, lᶻⁿ) - quadratic_loss(Z, lᶻᵈ)),
)
