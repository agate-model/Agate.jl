...
Components describe structure rather than ecological role. A `Plankton` declares prognostic `states` as a NamedTuple mapping each state identity to its conserved Element, using `nothing` for non-elemental states, plus one `reference_state`; for example, `states=(biomass=:carbon, chlorophyll=nothing)`. State identity is therefore independent of Element identity; the current contract permits at most one prognostic state per conserved Element. A plankton realizes one or more PFTs, and every PFT realizes one or more SizeClasses. `variable_states` is the derived set of all states other than the reference state. Each realized SizeClass carries one tracer per prognostic state: a one-state plankton uses identities such as `P_1` and `P_2`, while a multi-state plankton realizes state-qualified tracers such as `P_1_biomass` and `P_1_chlorophyll`. Interaction and plankton parameter axes are realized per parameter from its actual process applicability and therefore do not grow with state multiplicity. Setup resolves authored state identities to internal plankton-state references and compilation lowers those references to concrete static tracer operands, so no state-name lookup enters runtime kernels. `state_element` centrally reads the declared conserved-element bookkeeping identity for each state. Derived quantities such as N:C can then be expressed from prognostic state operands.
...
Parameter slots are resolved once during canonicalization and own their scientific numeric domain (`finite`, nonnegative, positive, or `[0,1]`). Shared parameter identity requires compatible semantic axes and domains. Construction validates fully realized storage against those domains plus coupled formulation constraints such as quota ordering and product budgets; the same coupled constraints are rechecked when active parameter vectors create a runtime wrapper or box ODE problem. `parameter_domains` exposes slot domains for inference workflows. Rank-2 parameters may use dense rectangular storage while binding-derived applicability records only the modeled interaction edges; introspection exposes those edges and active selection rejects unused Cartesian cells. Each canonical process carries dense references into the single ordered `ParameterBinding` tuple, arranged alongside its process, factor, product, and stoichiometry structure. Lowering consumes those references directly; scientific tree paths remain setup-time validation/error context rather than a second lookup key representation.
