...
2. **Configuration and construction** (`Configuration/` and `Construction/`).
   Agate normalizes community and interaction inputs, resolves parameters and derived matrices, and assembles the concrete biogeochemistry object. Recipes capture the authored scientific definition; manifests record the resolved scientific state.
