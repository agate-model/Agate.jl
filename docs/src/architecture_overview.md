...
Parameter slots are resolved once during canonicalization and own their scientific numeric domain (`finite`, nonnegative, positive, or `[0,1]`). Construction validates fully realized parameter storage against those domains, `parameter_domains` exposes them for inference workflows, and active parameter vectors are revalidated when a runtime wrapper or box ODE problem is created. Each canonical process carries dense references into the single ordered `ParameterBinding` tuple, arranged alongside its process, factor, product, and stoichiometry structure. Lowering consumes those references directly; scientific tree paths remain setup-time validation/error context rather than a second lookup key representation.
